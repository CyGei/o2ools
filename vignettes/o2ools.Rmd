...
To extract the individual trees from the posterior sample, we can use `get_trees()`. This function will return a list of trees, one for each sample in the posterior. You can add additional *outbreaker2* results to the trees, such as the infection times (`t_inf`), the number of generations (`kappa`) and other attributes from the linelist. Any attribute from the linelist has to be in the same order as the cases in the linelist.
...
## Augment the linelist with outbreaker's results
...
We'll include additional function to compute the case reproduction number and the serial interval. For additional analyses check the other vignettes in the package.
